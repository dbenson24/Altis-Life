...
if(!(["APP_fnc_",_functionName] call BIS_fnc_inString) && {(toLower(_functionName) in ["app_fnc_insertrequest"."app_fnc_queryrequest","app_fnc_playersessionend","app_fnc_playercreatesession","app_fnc_playergetsession","app_fnc_invhandler"])} && {!(toLower(_functionName) in ["bis_fnc_execvm","bis_fnc_effectkilledairdestruction","bis_fnc_effectkilledairdestructionstage2","life_fnc_stripDownPlayer"])}) exitWith {false};
